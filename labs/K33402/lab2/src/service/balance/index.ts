...
import Currancy, { CurrancyAtributes } from "../../models/currency";
...
class BalanceService {
    ...
    async create(userId: number, currencyId: number, basketData: Omit<BalanceAtributes, 'currency' | 'user'>){
        try{
            ...
            const result = await Balance.create({...basketData, userId: user.id, userName: user.name, currencyId: currancy.id, currency: currancy.name});
            return result;
        }catch (err){
            throw err;
        }
