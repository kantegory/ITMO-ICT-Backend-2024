...
class LocationActivityService {
    ...
    async getAllByActivityId(activityId: string): Promise<LocationActivity[]> {
        try {
            const locationActivities = await LocationActivity.findAll({ where: { activity_id: activityId } });
    ...
    async getAllByLocationId(locationId: string): Promise<LocationActivity[]> {
        try {
            const locationActivities = await LocationActivity.findAll({ where: { location_id: locationId } });
            return locationActivities;
        } catch (error) {
            throw error;
        }
