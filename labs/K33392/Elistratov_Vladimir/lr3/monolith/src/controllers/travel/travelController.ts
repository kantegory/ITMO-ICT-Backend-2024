...
class TravelController {
    ...
    listFindByNumberOfSeats = async (request: Request, response: Response) => {
        const seatsNumber = Number(request.params.seats_number)
        const travelList = await this.travelService.findAllByNumberOfSeats(seatsNumber)
        if (!travelList) {
            return response.status(404).send()
        }
